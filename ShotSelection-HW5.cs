...
namespace GameAIStudent
{
    ...
    public class ShotSelection
    {
            ...
            )
        {
            ...
            const float maxInterceptTime = 3f;
            if (interceptT > maxInterceptTime)
            {
                return SelectThrowReturn.NoThrowTargettingFailed;
            }
            ...
            if (deltaT > 0.001f) // Ensure we have valid delta time
            {
                // Calculate acceleration from velocity change
                Vector3 rawAcceleration = (opponent.Vel - opponent.PrevVel) / deltaT;
                if ((opponent.Vel - opponent.PrevVel).sqrMagnitude < 0.01f)
                {
                    rawAcceleration = Vector3.zero;
                }
                float accelerationThreshold = 0.3f; // m/s² threshold
                
                // Check if acceleration is significant
                if (rawAcceleration.magnitude > accelerationThreshold)
                {
                    return SelectThrowReturn.NoThrowOpponentCurrentlyAccelerating;
                }

                // Check for significant direction change
                if (opponent.PrevVel.magnitude > 0.1f && opponent.Vel.magnitude > 0.1f)
                {
                    float angleChange = Vector3.Angle(opponent.PrevVel.normalized, opponent.Vel.normalized);
                    if (angleChange > 15f) // degrees threshold
                    {
                        return SelectThrowReturn.NoThrowOpponentCurrentlyAccelerating;
                    }
                }
            ...
            Vector3 throwStartPos = thisMinion.HeldBallPosition;
            Vector3 throwDirection = interceptPos - throwStartPos;
            float throwDistance = throwDirection.magnitude;
            throwDirection.Normalize();

            // Cast two parallel rays to account for ball width
            float ballWidth = 0.2f; // Approximate ball radius
            Vector3 perpendicular = Vector3.Cross(throwDirection, Vector3.up).normalized * ballWidth;

            // First ray
            if (Physics.Raycast(throwStartPos + perpendicular,throwDirection,out RaycastHit hit1,throwDistance,
                mask))
            {
                if (Vector3.Distance(hit1.point, interceptPos) > ballWidth * 2f)
                {
            ...
            // Second ray
            if (Physics.Raycast(throwStartPos - perpendicular,throwDirection,out RaycastHit hit2,throwDistance,
                mask))
            {
                if (Vector3.Distance(hit2.point, interceptPos) > ballWidth * 2f)
                {
