    ...
    public class ThrowMethods
    {
            ...
            out float altT)
        {
            ...
            if (gravity.magnitude < Mathf.Epsilon)
            {
                ...
                float a = relativeVel.sqrMagnitude - maxProjectileSpeed * maxProjectileSpeed;
                ...
                interceptT = (t1 > 0f) ? t1 : ((t2 > 0f) ? t2 : -1f);
                if (interceptT <= 0f) return false;

                Vector3 predictedPos = targetInitPos + targetConstVel * interceptT;
                projectileDir = (predictedPos - projectilePos).normalized;
                projectileSpeed = maxProjectileSpeed * holdbackFactor;
                altT = interceptT;
                return true;
            }
