    ...
    public class ShotSelection
    {
            ...
            )
        {
            ...
            if (deltaT > 0.001f) // Ensure we have valid delta time
            {
                ...
                if (rawAcceleration.magnitude > 2f || // m/s² threshold
                    (opponent.PrevVel.magnitude > 1f && 
                    Vector3.Angle(opponent.PrevVel, opponent.Vel) > 30f)) // degrees threshold
                {
            ...
            int mask = Physics.AllLayers & ~(1 << Mgr.NavMeshCarverLayerIndex) 
               & ~(1 << Mgr.MinionTeamALayerIndex) & ~(1 << Mgr.MinionTeamBLayerIndex)
               & ~(1 << Mgr.BallTeamALayerIndex) & ~(1 << Mgr.BallTeamBLayerIndex);

            if (Physics.SphereCast(thisMinion.HeldBallPosition, 0.2f, 
                (interceptPos - thisMinion.HeldBallPosition).normalized,
                out _, Vector3.Distance(thisMinion.HeldBallPosition, interceptPos), mask))
            {
